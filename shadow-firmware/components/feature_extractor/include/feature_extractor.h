/*
 * ESP32-S3 Optimized Feature Extractor (ESP-IDF Version)
 ...
 * and in-place operations for ESP32-S3 deployment with FreeRTOS
 */
...
#define FEATURE_VECTOR_SIZE (NUM_FEATURES * sizeof(float))
...
// Feature extraction result (using float for ML model compatibility)
typedef struct {
    float features[NUM_FEATURES];          // 30 features as float for ML
    uint32_t extraction_time_ms;           // Time taken for extraction
    bool success;                          // Extraction status
    TickType_t timestamp;                  // When features were extracted (FreeRTOS ticks)
} feature_vector_t;

// Computation workspace for feature extraction
typedef struct {
    fixed_point_t workspace[BVP_BUFFER_SIZE * 2];  // Workspace for sorting + temp space
...
// Fixed-point math utilities (optimized for ESP32-S3)
