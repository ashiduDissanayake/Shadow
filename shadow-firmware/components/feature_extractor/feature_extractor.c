/*
 * ESP32-S3 Optimized Feature Extractor Implementation (ESP-IDF Version)
 * 
 * High-performance feature extraction using fixed-point arithmetic
 * and memory-efficient algorithms optimized for ESP32-S3 with FreeRTOS
 */
...
static const char *TAG = "FeatureExtractor";
...
int extract_bvp_features(multi_sensor_buffer_t *msb,
    ...
    // Store BVP features (convert to float for ML model)
    features[0] = FIXED_TO_FLOAT(stats.mean);    // BVP_MEAN
    features[1] = FIXED_TO_FLOAT(stats.std);     // BVP_STD
    features[2] = FIXED_TO_FLOAT(stats.min);     // BVP_MIN
    features[3] = FIXED_TO_FLOAT(stats.max);     // BVP_MAX
    features[4] = FIXED_TO_FLOAT(stats.median);  // BVP_MEDIAN
    features[5] = FIXED_TO_FLOAT(stats.range);   // BVP_RANGE
    features[6] = FIXED_TO_FLOAT(stats.iqr);     // BVP_IQR
    features[7] = FIXED_TO_FLOAT(stats.energy);  // BVP_ENERGY
...
int extract_acc_features(multi_sensor_buffer_t *msb,
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        // Store ACC features for this axis (5 features per axis, convert to float)
        int base_idx = axis * 5;
        features[base_idx + 0] = FIXED_TO_FLOAT(stats.mean);   // ACC_X/Y/Z_MEAN
        features[base_idx + 1] = FIXED_TO_FLOAT(stats.std);    // ACC_X/Y/Z_STD
        features[base_idx + 2] = FIXED_TO_FLOAT(stats.min);    // ACC_X/Y/Z_MIN
        features[base_idx + 3] = FIXED_TO_FLOAT(stats.max);    // ACC_X/Y/Z_MAX
        features[base_idx + 4] = FIXED_TO_FLOAT(stats.energy); // ACC_X/Y/Z_ENERGY
    }
...
int extract_eda_features(multi_sensor_buffer_t *msb,
    ...
    // Store EDA features (convert to float)
    features[0] = FIXED_TO_FLOAT(mean);                    // EDA_MEAN
    features[1] = FIXED_TO_FLOAT(std);                     // EDA_STD
    features[2] = FIXED_TO_FLOAT(min_val);                 // EDA_MIN
    features[3] = FIXED_TO_FLOAT(max_val);                 // EDA_MAX
...
int extract_temp_features(multi_sensor_buffer_t *msb,
    ...
    // Store TEMP features (convert to float)
    features[0] = FIXED_TO_FLOAT(mean);                    // TEMP_MEAN
    features[1] = FIXED_TO_FLOAT(std);                     // TEMP_STD
    features[2] = FIXED_TO_FLOAT(range);                   // TEMP_RANGE
...
void print_feature_vector(const feature_vector_t *features) {
    ...
    ESP_LOGI(TAG, "Status: %s | Extraction time: %u ms", 
    ...
    for (int i = 0; i < NUM_FEATURES; i++) {
        ESP_LOGI(TAG, "  [%2d] %-12s = %8.4f", i, feature_names[i], 
               features->features[i]);
    }
}
