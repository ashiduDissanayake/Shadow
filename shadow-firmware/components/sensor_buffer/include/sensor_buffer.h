/*
 * ESP32-S3 Multi-Sensor Circular Buffer System (ESP-IDF Version)
 * 
 * 6-Layer Circular Buffer for Synchronized Sensor Data Collection
 * Optimized for ESP32-S3 with FreeRTOS thread safety
 * 
 * Buffer Layout:
...
typedef struct {
    ...
    TickType_t last_sample_time;   // Last sample timestamp (FreeRTOS ticks)
} sensor_buffer_t;

// Main multi-sensor buffer system with FreeRTOS synchronization
typedef struct {
    sensor_buffer_t layers[NUM_SENSOR_LAYERS];
    SemaphoreHandle_t mutex;       // Thread-safe access
    TickType_t system_start_time;  // System start timestamp
    uint8_t initialized;
...
// Data collection functions (thread-safe)
...
// Data retrieval functions (thread-safe)
...
// Timing and synchronization (FreeRTOS-based)
